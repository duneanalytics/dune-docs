...
The websocket complement that by allowing you to subscribe to balance changes in real-time, without the need to poll the API.

<Warning>Native tokens are currently not supported. This means that balance changes to them will not trigger an websocket push</Warning>
  ...
  <Tab title="Terminal">
    <Steps>
      ...
      <Step title="Call the webhook">
        Just replace the Wallet Address and Dune API Key with your own values.
        ```bash Terminal
