...
<Accordion title="Step 2: Is it created by a factory?">
  ...
  <Check>
    **If YES:**
    - Enable "Decode other contracts created by the same factory" flag
...
- **Tip:** If an ABI isn't verified for certain addresses, the ABI will be the same across chains in the same project (i.e., UniswapV2Pair ABI is going to be the same on all EVM chains regardless of the address)
