...
>
  <iframe
    src="https://demo.arcade.software/wEVEG2p4ns4oXV5LSpJ3?embed"
...
Examples of this include parameters that allow users to choose from a list of tokens or a list of token pairs.

![list parameter](/web-app/query-editor/images/parameters/list-parameter.jpeg)
...
```sql
--- list parameter allowing multiple values
Select * from dex.trades
where token in (Select token from unnest(split('{{token_list_parameter}}',',')) as c(token))
```

If you want to allow multiple values, you need to adjust your query for this. Use the `unnest` and `split` functions to turn the columns into usable values.
...
*/
``` 
Most of the time, you'll want to use the values in a `where in` clause. To do this, you need to use the `unnest` and `split` functions to turn the columns into usable values.
For example:
```sql
Select * from dex.trades
...
When you manually define a list of values, the list can be text values, numbers, dates or even varbinary values. The system attempts to detect the type of the list values, but sometimes you might have to manually adjust the type within the query by using the `cast` function.
...
Filter on all available perpetual futures available on synthetix. The master query for this is query_3256054. We can use this query to populate a list of all available perps:
