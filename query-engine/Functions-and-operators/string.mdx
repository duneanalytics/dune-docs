...
Splits `string` on `delimiter` and returns an array of size at most `limit`. The last element in the array always contains everything left in the `string`. `limit` must be a positive number.
...
<tip> This SQL-standard function has special syntax and requires specifying `form` as a keyword, not as a string. </tip>
