...
<Note>Trino does not support JSON objects with duplicate keys.</Note>
...
SELECT json_object(
                   x : [  "text"  ]  FORMAT JSON,
                   y : X5B0035005D00 FORMAT JSON ENCODING UTF16
                  )
--> {"x":["text"],"y":[5]}
```
...
--> X7b 00 22 00 78 00 22 00 3a 00 31 00 7d 00

SELECT json_object(x : 1 RETURNING VARBINARY FORMAT JSON ENCODING UTF32)
--> X7b 00 00 00 22 00 00 00 78 00 00 00 22 00 00 00 3a 00 00 00 31 00 00 00 7d 00 00 00
```
...
If the element at the specified index doesnt exist, the function returns null:
...
The link`json_query function<json-query>` provides a more powerful and
