...
description: Use SVM-based classifiers and regressors directly within DuneSQL.
---
...
processing. The capability to train large data sets is limited by the
...
is an N-dimensional vector whose elements are numerical values. In
...
`classify` returns the predicted label by using the trained model. The trained model cannot be saved natively,
...
a continuous value, unlike the classification problem. The target must be
