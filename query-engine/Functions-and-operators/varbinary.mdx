...
Dune SQL represents varbinaries using the varbinary type.

To make it simpler to work with varbinaries we have the following helper functions, which work with these two kinds of representation. They simplify interactions with varbinaries, as they automatically account for the `0x`-prefix and use byte index instead of character index. For instance, the varbinary_substring methods take indexes by byte, not by character (twice the varbinary length).


You can use these functions to extract data from undecoded events logs, or calldata. For instance, if you have a function that takes a `uint256` as an argument, you can use `varbinary_to_uint256` to extract the value from the calldata. You'll oftentimes need to use `varbinary_substring` to extract the correct part of the calldata or use other bytearray manipulation functions to get the correct value.
...
<tip> Varbinary functions were previously called byte array functions. You might find e.g. `bytearray_to_uint256` in some of our older queries. These functions are identical to the varbinary functions and work as aliases. </tip>
...
<Warning> You **cannot** use ``cast(x as varbinary)`` in these cases as it will actually encode the string as varbinary which is different from converting the expression to `varbinary`.  </Warning>
...
Concatenates two varbinaries or strings.
...
```sql
 ...
 -- returning 0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2
SELECT varbinary_rtrim(0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc20000000000000000000000000000000000000000)
```
...
```sql
-- using varbinary_substring starting from the 21st index
