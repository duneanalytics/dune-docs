...
It also makes a difference whether we join `ethereum.transactions` on `uniswap_v3_ethereum.Pair_evt_Swap` or the other way around. The following query will be even faster:
...
7. **Optimize subqueries**: Subqueries (Select * from (select * from table) as x) can sometimes cause performance issues. Consider using Common Table Expressions (CTEs) or rewriting the query using JOINs to optimize subqueries.
...
9. **Only order when necessary**: Ordering results can be computationally expensive. If you don't need ordered results, avoid using ORDER BY.

10. **Use `UNION ALL` instead of `UNION`**: If you're combining the results of multiple queries, use `UNION ALL` instead of `UNION` to avoid the overhead of removing duplicate rows.
