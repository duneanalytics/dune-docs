...
<info> Using `block_date` is often better than using `block_number` or `block_time` as there are less distinct values to check against. Use `block_date` during joins where  it is possible.{" "} </info>
