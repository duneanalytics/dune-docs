...
Implicit Conversion and Casting to other datatypes is described in [the conversion function documentation](/query-app/query-engine/Functions-and-operators/conversion).
...
We have built custom functions to make it easier to work with varbinaries in DuneSQL. Check the [varbinary functions](/query-app/query-engine/Functions-and-operators/varbinary) page for more information.
...
See also [`date and time functions`](/query-app/query-engine/Functions-and-operators/datetime).
...
using the _IPv4-mapped IPv6 address_ range
...
using the canonical format defined in `5952`\{.interpreted-textrole="rfc"}.
