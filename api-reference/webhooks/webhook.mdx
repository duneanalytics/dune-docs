...
```
<Note>
You'll need to GET query the `data_uri` with your api key to get the results data. This is the same as passing the `execution_id` into [get execution result](/api-reference/query-api/endpoint/get-execution-result).
</Note>

<Warning>
In the case the query fails to run, you won't recieve anything to the webhook.
</Warning>