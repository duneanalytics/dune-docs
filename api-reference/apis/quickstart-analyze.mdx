...
<Note>
The `execute()` function in our SDKs automatically handles polling for query completion and pagination. With cURL, you'll need to poll the execution status endpoint and then fetch results manually.
</Note>
...
for row in results.rows:
    print(f"{row['protocol']}: ${row['volume']:,.2f}")
```
...
def daily_dex_analysis():
    ...
    WHERE block_time > now() - interval '24' hour
    """
    volume_data = dune.execute(sql=volume_query)
    ...
    LIMIT 10
    """
    protocol_data = dune.execute(sql=protocol_query)
    ...
    ORDER BY 2 DESC
    """
    chain_data = dune.execute(sql=chain_query)
    
    # Combine and process results
    report = {
        'timestamp': datetime.now(),
        'total_volume': volume_data.rows[0]['total_volume'],
        'unique_traders': volume_data.rows[0]['unique_traders'],
        'top_protocols': pd.DataFrame(protocol_data.rows),
        'chain_breakdown': pd.DataFrame(chain_data.rows)
    }
...
<AccordionGroup>
  <Accordion title="Use the appropriate performance tier">
    ...
    ```python
    results = dune.execute(sql=sql, performance="large")
    ```
  </Accordion>
  ...
  <Accordion title="Leverage materialized views">
    ...
    results = dune.execute(sql="SELECT * FROM daily_dex_volume")
    ```
  </Accordion>
