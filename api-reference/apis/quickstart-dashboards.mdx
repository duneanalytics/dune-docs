...
<CodeGroup>
  ...
  npm install @duneanalytics/client-sdk@^0.2.5
  ```

  ```bash Python
  pip install dune-client>=1.3.0
  ```
</CodeGroup>
...
def get_dex_volume():
    ...
    results = dune.run_query(query_id=3493826)
    return jsonify({
...
<Note>
  **Important Limitations:**
  ...
  - **Python SDK:** Direct SQL execution with `dune.run_sql()` requires a Dune Plus subscription. For most users, use saved queries with `dune.run_query(query_id=123)`.
...
def build_comprehensive_dashboard():
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = {
            name: executor.submit(dune.run_query, query_id=query_id)
