  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const executionResult = await dune.runQuery(3493826);
...
<Note>
  **Note:** The TypeScript SDK (`@duneanalytics/client-sdk`) only supports saved queries, not direct SQL execution. For SQL queries in JavaScript/TypeScript, use saved queries with `dune.runQuery(queryId)` or use the REST API directly with `fetch()`.
</Note>
...
function ParameterizedDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const executionResult = await dune.runQuery(queryId, params);
      setData(executionResult.result?.rows);
    }
  ...
  useEffect(() => {
    async function loadDashboard() {
      ...
      const [volume, users, protocols] = await Promise.all([
        dune.runQuery(volumeQueryId),
        dune.runQuery(usersQueryId),
        dune.runQuery(protocolsQueryId)
      ]);
