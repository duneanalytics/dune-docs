...
- `FAILED`: One or more nodes in the pipeline failed
...
- `node_executions`: Array of individual node executions with their status
- `error`: Error message if the pipeline execution failed
...
<Note>
Once a pipeline execution completes (successfully or with failures), you can retrieve the results of individual queries using their respective execution IDs from the node status.
</Note>
