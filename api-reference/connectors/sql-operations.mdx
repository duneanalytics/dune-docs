...
While [dbt](/api-reference/connectors/dbt-connector) is the recommended tool for data transformations, these SQL operations work with any Trino-compatible client including Hex, Jupyter notebooks, SQLMesh, or direct Trino connections.
...
**Returns**: All public schemas plus your private schemas.
...
**Supported column types**: All DuneSQL data types including `BIGINT`, `DOUBLE`, `VARCHAR`, `VARBINARY`, `TIMESTAMP`, `BOOLEAN`, `ARRAY`, `MAP`, etc.
...
Synchronize partition information for a table.

**Example:**

```sql
CALL system.sync_partition_metadata('dune', 'your_team', 'partitioned_table');
```

**When to use**: Rarely needed. Used when partition metadata becomes inconsistent.
...
- [dbt Connector](/api-reference/connectors/dbt-connector)
- [Trino Connector](/api-reference/connectors/trino-connector)
