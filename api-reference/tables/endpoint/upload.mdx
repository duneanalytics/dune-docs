...
function uploadToDune() {
  var apiKey = "YOUR_DUNE_API_KEY"; // Replace with your actual Dune API key
  var tableName = "ten_year_us_interest_rates"; // Update with your desired table name
  var description = "10 year daily interest rates, sourced from https://fred.stlouisfed.org/series/DGS10";
  ...
  var csvData = values.map(row => row.join(",")).join("\n");
