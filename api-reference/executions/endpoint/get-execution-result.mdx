...
Welcome to the Examples and Tutorials section! Here, you'll find a collection of code examples and step-by-step tutorials to help you get started with getting execution results.

### Step-by-Step Tutorial: Getting Execution Results

#### Step 1: Obtain the Execution ID

Before you can get execution results, you need to have the `execution_id`. This ID is obtained from making an [execute query](/api-reference/executions/endpoint/execute-query) POST request.

#### Step 2: Get the Execution Results

Once you have the `execution_id`, you can get the execution results by making a GET request to the results endpoint.

**Example using cURL:**

```bash
curl -X GET -H x-dune-api-key:{{api_key}} "https://api.dune.com/api/v1/execution/{{execution_id}}/results"
```
