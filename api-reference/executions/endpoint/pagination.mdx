...
title: Pagination
---
All API endpoints to read query results, including [get execution results](./get-execution-result), [get execution results csv](./get-execution-result-csv), 
[get query results](./get-query-result), [get query results csv](./get-query-result-csv), support pagination for efficient data retrieval. Pagination divides 
large datasets into smaller, manageable chunks, preventing overload and ensuring smooth performance. This allows users to navigate through the data easily, 
avoiding limit errors and streamlining their data fetching experience.
...
- You can use the [Get Status API](./get-execution-status) to check the size of your result, `result.result_set_size`
