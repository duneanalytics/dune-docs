...
Results are linked to executions, and executions are tied to queries. Each execution has an `execution id`. An execution ID is generated when you initiate the execution of a query through the [execute query](./execute-query) endpoint.

Unlike a query ID, which identifies a specific query, an execution ID represents a particular execution of that query. With an execution ID, you can monitor the execution's status using the [check status](./get-execution-status) endpoint. The status can be one of the following: pending, success, or failure.

If an execution is successful, you can get the result using [get result](./get-execution-result). Results are saved and can be retrieved multiple times.
