...
Results are linked to executions, and executions are tied to queries. Each execution has an `execution id`. An execution ID is generated when you initiate the execution of a query through the [execute query](../query-api//endpoint/execute-query) endpoint.
