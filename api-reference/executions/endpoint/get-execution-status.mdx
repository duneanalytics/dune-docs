...
Welcome to the Examples and Tutorials section! Here, you'll find a collection of code examples and step-by-step tutorials to help you get started with checking execution status.

### Step-by-Step Tutorial: Checking Execution Status

#### Step 1: Obtain the Execution ID

Before you can check the execution status, you need to have the `execution_id`. This ID is obtained from making an [execute query](/api-reference/executions/endpoint/execute-query) POST request.

#### Step 2: Check the Execution Status

Once you have the `execution_id`, you can check the execution status by making a GET request to the status endpoint.

**Example using cURL:**

```bash
curl -X GET -H x-dune-api-key:{{api_key}} "https://api.dune.com/api/v1/execution/{{execution_id}}/status"
```
