...
3. If you are fetching JSON response, check the response headers for `next_offset` and `next_uri` to fetch the next page, if available. If you are fetching CSV response, check the response headers for `x-dune-next-offset` and `x-dune-next-uri` to fetch the next page, if available.
