...
All queries run via the connector are executed on the [Large Query Engine](/query-engine/query-executions#query-engine-size) and cost 20 Dune Credits per execution. In addition, fetching results consumes credits proportional to the number of datapoints returned. For a full breakdown, please see our documentation on [Billing](/api-reference/overview/billing).
