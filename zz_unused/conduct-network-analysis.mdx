...
<Note>
    ...
    - Credit consumption for each Dune API action is estimated and printed out with a ⛽ symbol. To learn more about credit, [please visit this page](learning/how-tos/credit-system-on-dune).
...
- These interactions are parameterized variables, adjust as needed.
...
<Tabs>
  ...
  <Tab title="Strongly Connected Components">
    ```py
...
- In Gephi: [Gephi](https://gephi.org/) is a good tool for presenting large networks comprehensively. Export the graph from Python with nx.write_gml(G, "file_name.gml") and dive into Gephi. Tweak node sizes, colors, and labels to distill insights.
...
<Tabs>
  <Tab title="Random Node Sampling">
    ```py
  ...
  <Tab title="Random Walk Sampling">
...
<Tabs>
  ...
  <Tab title="Example community detection">
    ```py
