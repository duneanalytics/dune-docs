...
Here, we will walk through two examples. Example 1 is to graph a Sankey for a certain subset of RabbitHole Quests with Optimism grant, to see how the questors are retained after a 1-month period. Example 2 is to graph a Sankey for the token flow when swapping USDC for WETH using Uniswap if the swap is not direct.

We will show you different ways to fetch data from the Dune API in the process as well.
...
<Note>
  1. Make sure you have a .env file created with your 'DUNE_API_KEY' in
