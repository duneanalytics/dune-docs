...
This guide provides a step-by-step process on using typescript and Dune API to create a user recommendation frame for Farcaster.

Here is the [live example](https://warpcast.com/~/conversations/0x62b84d8405fe6e9ff1639055852d98f910b4c595).

![frames_recording.gif](/learning/how-tos/images/frames_recording.gif)

## Set up your environment with the example repo

[Fork this repo](https://github.com/andrewhong5297/dune-frames) and then run `npm install -g frog` and `npm install` to get all your packages. If you don't have npm set up, then [go download node js first](https://nodejs.org/en/download).

Once installations are finished, try running `npm run dev` and it should take you to a localhost dev environment. Now, you'll need to get a Dune API key from [your user settings](https://dune.com/settings/api) and set it in an `.env` file.

With that done, you'll be able to run the frame - click "See From Your Followers" in your localhost environment.
