...
You can export data from Dune using two primary methods: CSV downloads and the API. Each offers different capabilities and file formats:
...
- **CSV Exports:** Available only to users on [Plus and Premium plans](https://dune.com/pricing). Exporting data as CSV involves a fixed credit charge per file download, depending on the size and complexity of the data extracted.
   - **Plus Plan:** 5 credits per 25,000 data points.
   - **Premium Plan:** 1 credit per 25,000 data points.
- **API Exports:** Available across all plans, but with varying rate limits and credit costs. Users can retrieve data from the API in either JSON or CSV format.
