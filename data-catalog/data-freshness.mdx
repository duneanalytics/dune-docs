...
# Abstractions
Abstractions on Dune are tables like `dex.trades` or `nft.trades` that are derived from the decoded data. They take away the complexity of working with raw or even decoded data and are ready to be used for analysis.
