...
description: Echelon blockchain transaction messages data
---

# Table Description

This table contains data about messages contained within transactions in the Echelon blockchain, including message types, content, and associated metadata.

<Tip>
On execution, transactions from Cosmos emit data in two formats:
- Messages (`echelon.tx_messages`): These provide a high-level, readable summary of actions, such as "send money" or "execute contract." They are easier to understand and track transaction types.
- Events (`echelon.message_events`): These capture detailed, low-level changes at the chain level, including underlying blockchain modifications that may not always be directly related to state changes.
...
<TableSample tableSchema="echelon" tableName="tx_messages" />
