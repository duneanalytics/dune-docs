...
<Warning>
**Important**: The USD values in `tokens.transfers` are calculated using `prices_coinpaprika.hour`, **not** the hybrid `prices.hour` table.
</Warning>
...
Note: The `amount_usd` values are calculated using Coinpaprika pricing data (`prices_coinpaprika.hour`) for stable, exchange-validated pricing.
