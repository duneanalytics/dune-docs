...
2. **Data processing pipeline**:
   ...
   - **Minute-level data uses interpolated prices**: The `prices.minute` table now uses linear interpolation between hourly anchor points from `prices.hour` to provide smooth, continuous minute-level data while reducing noise and outliers
...
- **Quality filtering is then applied to reduce the impact of anomalous trades:**
  ...
  - **Volume Outlier Detection**: Trade volumes below $0.01 or above $1,000,000
...
4. **Volume Outlier Detection**:
   - Trades with volumes below $0.01 or above $1,000,000
   - This helps filter out dust trades and suspiciously large volumes

A trade is flagged as an outlier if it triggers **any** of these four conditions. Flagged trades are excluded from price calculations to improve data quality.
