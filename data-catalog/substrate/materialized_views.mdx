...
description: Useful materialized views for Polkadot, Kusama & their parachains.
---

<Info>
Materialized views store pre-computed query results as tables, enabling faster queries and simplified data pipelines. To learn more about creating and using materialized views, see the [Materialized Views page](/query-engine/materialized-views).
</Info>

A list of useful queries that we have [materialized](https://dune.com/queries?category=third_party_data&catalog=dune&schema=substrate) for usability. Some complex dashboards can be constructed more efficiently by referencing these materialized views. Forks and additions are welcome!
## Polkadot
